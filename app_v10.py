...
#: 민감도 단계별 임계값.
#:
#: ``score``    : 최종 점수 하한. 이 아래는 정식 결과에서 뺀다.
#: ``intent``   : 통과에 필요한 의도 신호(INTENT_ROWS) 개수.
#: ``reaction`` : 통과에 필요한 반응 신호(REACTION_ROWS) 개수.
#:
#: '민감' 단계만 반응을 요구하지 않는다. 근거가 약한 구간까지 넓게 훑어보는
#: 용도이므로, 대신 그렇게 올라온 구간은 점수가 낮게 남는다.
#:
#: 같은 영상이라도 현장 상황에 따라 걸러 내는 정도가 달라야 한다. 관제 인력이
#: 넉넉하면 넓게 보고(민감), 확인할 여력이 없으면 확실한 것만 본다(엄격).
#: 판정 로직을 건드리지 않고 관문의 세기만 바꾸는 방식이라, 어떤 단계에서도
#: 점수 계산과 근거 표시는 동일하다.
SENSITIVITY_LEVELS: tuple[str, ...] = ("엄격", "보통", "민감")

#: 의도 신호 중에서도 '반응'에 해당하는 항목.
#:
#: 지연 경로 유사도는 의도 신호로 묶여 있지만 반응은 아니다. 뒤에서 같은
#: 속도로 걸으면 선행자가 지나간 자리를 그대로 지나가므로 값이 100%까지
#: 오른다 — 같은 길을 걸은 결과일 뿐 따라붙을 의도의 증거가 아니다.
#:
#: 반면 정지 후 추종과 방향 전환 추종은 선행자의 행동을 보고 시간차를 두고
#: 맞춰야 채워진다. 우연히 같은 길을 가는 보행자와 미행을 가르는 신호는
#: 이 둘뿐이므로, 관문이 별도로 요구할 수 있게 따로 둔다.
REACTION_ROWS: tuple[str, ...] = ("정지 후 추종", "방향 전환 추종")

SENSITIVITY_THRESHOLDS: dict[str, dict[str, int]] = {
    "엄격": {"score": 55, "intent": 2, "reaction": 1},
    "보통": {"score": 40, "intent": 1, "reaction": 1},   # score는 MIN_EVENT_SCORE와 같은 값
    "민감": {"score": 25, "intent": 1, "reaction": 0},
}

DEFAULT_SENSITIVITY = "보통"

SENSITIVITY_HINTS: dict[str, str] = {
    "엄격": "확실한 근거가 있는 구간만 올립니다.",
    "보통": "기본값입니다. 대부분의 상황에 맞습니다.",
    "민감": "낮은 점수까지 올립니다. 오탐이 늘어납니다.",
}


def sensitivity_thresholds(level) -> dict[str, int]:
    """민감도 이름으로 임계값을 찾는다. 모르는 값이면 기본값을 쓴다.

    저장된 기록을 다시 열 때 예전 버전이 남긴 이름이 들어올 수 있어서,
    사전에 없는 값이 와도 화면이 죽지 않게 기본값으로 되돌린다.
    """
    return SENSITIVITY_THRESHOLDS.get(str(level), SENSITIVITY_THRESHOLDS[DEFAULT_SENSITIVITY])
...
class Event:
    ...
    #: 관문에 걸린 이유. 정식 이벤트는 빈 문자열이고, 참고 이벤트에만 값이
    #: 들어간다. 화면에서 "왜 이 쌍이 목록에 없는가"를 설명하는 데 쓴다.
    reason: str = ""
...
class AnalysisRecord:
    ...
    #: 관문에 걸려 정식 목록에 오르지 못한 쌍. 계산은 끝난 상태이며, 정식
    #: 결과가 하나도 없을 때 참고용으로 화면에 대신 표시한다.
    aside_events: tuple[Event, ...] = ()
    #: 분석에 쓴 민감도 단계. 기록을 다시 열 때 어떤 기준이었는지 보여 준다.
...
def record_to_dict(rec: AnalysisRecord) -> dict:
    ...
    return {
        ...
        "sensitivity": str(getattr(rec, "sensitivity", DEFAULT_SENSITIVITY)),
        "events": [event_to_dict(e) for e in rec.events],
        "aside_events": [event_to_dict(e) for e in getattr(rec, "aside_events", ())],
    }
...
def record_from_dict(d: dict) -> AnalysisRecord:
    ...
    return AnalysisRecord(
        ...
        aside_events=tuple(event_from_dict(e) for e in d.get("aside_events", ())),
        sensitivity=str(d.get("sensitivity", DEFAULT_SENSITIVITY)),
    )
...
class AnalysisResult:
    ...
    #: 관문에 걸려 정식 목록에 오르지 못한 쌍. 계산은 이미 끝났으므로 버리지
    #: 않고 넘겨, 정식 결과가 없을 때 화면이 참고용으로 보여 줄 수 있게 한다.
    aside_events: tuple[Event, ...] = ()


def analyze_video(video_path, progress=None, total_frames=None, duration_sec=None,
                  fps=None, sensitivity=DEFAULT_SENSITIVITY) -> AnalysisResult:
    ...
    events, aside = compute_events(tracks, use_fps, span_sec, sensitivity)
    # 박스 색 기준은 compute_events가 쓴 것과 같은 임계값이어야 한다. 여기에
    # 숫자를 다시 적어 두면 두 기준이 조용히 갈라진다(요구사항 1.2).
    cut = sensitivity_thresholds(sensitivity)["score"]
    danger_ids = {e.follower for e in events if e.score >= cut}
    ...
    if rendered is None:
        return AnalysisResult(events=tuple(events), video_path=str(video_path),
                              render_failed=True, aside_events=tuple(aside))
...
#: 결과로 남길 최소 위험도 점수.
#: 기본 민감도('보통')의 점수 하한. SENSITIVITY_THRESHOLDS["보통"]["score"]와
#: 같은 값이어야 한다 — 아래 단언이 두 값이 갈라지는 순간 바로 알려 준다.
MIN_EVENT_SCORE = 40
assert MIN_EVENT_SCORE == SENSITIVITY_THRESHOLDS[DEFAULT_SENSITIVITY]["score"]
...
def reaction_evidence_count(rows) -> int:
    """반응 신호(REACTION_ROWS) 중 점수가 0보다 큰 항목의 개수.

    선행자의 정지·방향 전환에 시간차를 두고 맞춘 흔적이 몇 개인지를 센다.
    이 값이 0이면 거리·방향·경로가 아무리 유사해도 "같은 길을 가는 사람"과
    구별할 근거가 없다.
    """
    return sum(1 for r in rows if r.name in REACTION_ROWS and int(r.score) > 0)


def intent_evidence_count(rows) -> int:
    """의도 신호(INTENT_ROWS) 중 점수가 0보다 큰 항목의 개수.

    :func:`has_intent_evidence`가 "있는가"만 답한다면 이 함수는 "몇 개인가"를
    답한다. 민감도 관문이 개수를 요구하기 때문에 필요하다(엄격 단계는 2개).
    """
    return sum(1 for r in rows if r.name in INTENT_ROWS and int(r.score) > 0)
...
def compute_events(tracks, fps, duration_sec, sensitivity=DEFAULT_SENSITIVITY):
    """트랙 좌표에서 이벤트를 만든다.
    ...
    ``sensitivity``는 관문의 세기를 정한다(:data:`SENSITIVITY_THRESHOLDS`).
    판정 계산 자체는 단계와 무관하게 같고, 통과 기준만 달라진다.

    반환은 ``(정식 이벤트, 참고 이벤트)`` 두 묶음이다. 관문에 걸린 쌍도 계산은
    이미 끝난 상태이므로 버리지 않고 참고 묶음에 담는다. 화면이 "감지되지
    않았습니다"로 끝나지 않고 무엇을 왜 걸렀는지 보여 줄 수 있어야 하기
    때문이다. 참고 이벤트에는 :attr:`Event.reason`에 제외 사유가 들어간다.
    """
    th = sensitivity_thresholds(sensitivity)
    ids = list(tracks)
    out, aside = [], []
    for leader in ids:
        for follower in ids:
            ...
            # 관문은 셋이다. 순서가 곧 근거의 세기다.
            #  (1) 반응 신호 — 선행자의 정지·방향 전환에 시간차를 두고 맞춘
            #      흔적. 이게 없으면 거리·방향·경로가 아무리 유사해도 "같은
            #      길을 가는 사람"과 구별할 방법이 없다.
            #  (2) 의도 신호 개수 — 반응에 지연 경로 유사도까지 포함한 넓은 묶음.
            #  (3) 합계 점수.
            # 어느 관문에 걸렸는지를 사유로 남겨 화면에서 설명한다.
            reaction = reaction_evidence_count(rows)
            if reaction < th["reaction"]:
                aside.append(replace(event, reason="반응 근거 없음"))
                continue
            intent = intent_evidence_count(rows)
            if intent < th["intent"]:
                aside.append(replace(event, reason=f"의도 신호 부족 ({intent}개)"))
                continue
            if fin < th["score"]:
                aside.append(replace(event, reason=f"점수 미달 ({fin}점)"))
                continue
            out.append(event)
    ...
    return _dedupe(out), _dedupe(aside)
...
def page_analyze():
    ...
    if not rec.events:
        aside = tuple(getattr(rec, "aside_events", ()) or ())
        if not aside:
            st.success(NO_EVENT_NOTE)
            # 결과가 없어도 영상은 보여 준다. 분석이 끝난 영상을 확인조차 할 수
            # 없으면 "무엇을 근거로 없다고 하는지" 확인할 방법이 사라진다.
            if getattr(rec, "video_available", False):
                video_player(rec.video_path, 0,
                             st.session_state.get("seek_nonce", 0))
            return
        aside_notice(rec, aside)
        # 참고 구간을 정식 이벤트 자리에 넣어 같은 화면 구성을 그대로 쓴다.
        # 점수·근거·타임라인 계산은 이미 끝나 있으므로 표시 경로가 다를 이유가 없다.
        render_blocks(build_view(replace(rec, events=aside)))
        return

    render_blocks(build_view(rec))


#: 참고 구간을 대신 보여 줄 때의 안내. 정식 결과가 아니라는 점을 먼저 밝힌다.
ASIDE_TITLE = "확인이 필요한 구간은 감지되지 않았습니다"
ASIDE_BODY = ("아래는 관문에 걸려 정식 목록에 오르지 못한 구간입니다. "
              "계산은 끝난 상태이며, 참고용으로 표시합니다.")
ASIDE_HINT = "사이드바의 민감도를 올리면 정식 결과로 표시될 수 있습니다."


def aside_notice(record, aside) -> None:
    """참고 구간을 보여 주기 전에 띄우는 안내.

    "감지되지 않았다"와 "아래에 무언가 보인다"가 한 화면에 함께 있으면 오해를
    부르므로, 무엇을 왜 보여 주는지와 제외 사유를 먼저 적는다.
    """
    reasons = [e.reason for e in aside if getattr(e, "reason", "")]
    top = reasons[0] if reasons else "기준 미달"
    level = str(getattr(record, "sensitivity", DEFAULT_SENSITIVITY))
    st.markdown(
        f"<div class='zk-card' style='border-left:5px solid {RISK_LOW.base};"
        f"padding:16px 18px;margin-bottom:14px'>"
        f"<div style='font-size:15px;font-weight:700;color:{RISK_LOW.deep}'>"
        f"{ASIDE_TITLE}</div>"
        f"<div style='font-size:13px;color:{INK_SOFT};margin-top:7px;line-height:1.7'>"
        f"{ASIDE_BODY}<br>"
        f"현재 민감도 <b>{level}</b> · 최상위 구간 제외 사유 <b>{top}</b><br>"
        f"{ASIDE_HINT}</div></div>", unsafe_allow_html=True)
...
def current_sensitivity() -> str:
    """세션에 담긴 민감도 단계. 없거나 모르는 값이면 기본값으로 되돌린다."""
    picked = st.session_state.get("sensitivity", DEFAULT_SENSITIVITY)
    return picked if picked in SENSITIVITY_THRESHOLDS else DEFAULT_SENSITIVITY


def sidebar_sensitivity() -> str:
    """민감도 선택 수단. 확정된 단계 이름을 돌려준다.

    분석_모드와 나란히 두는 이유는 둘 다 "다음 분석이 어떻게 돌아갈지"를 정하는
    설정이기 때문이다. 이미 끝난 기록에는 영향을 주지 않는다 — 기록에는 분석
    당시의 단계가 함께 저장된다.
    """
    current = current_sensitivity()
    if st.session_state.get("sensitivity") != current:
        st.session_state.sensitivity = current

    picked = st.sidebar.select_slider(
        "민감도", options=list(SENSITIVITY_LEVELS), value=current)
    if picked != current:
        st.session_state.sensitivity = picked
        st.rerun()

    st.sidebar.caption(SENSITIVITY_HINTS[picked])
    return picked
