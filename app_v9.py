"""
집킴이 (Zipkimi) v9 - CCTV 미행 가능성 선별 시스템
...
def level(s):
    ...
    if s>=20: return "의심 낮음",MINT,MINT_PALE,MINT_DEEP
...
def analyze_video(path, prog=None, sens="보통"):
    import cv2
    ...
    events = compute(tracks, FPS, sens)
    diag = getattr(compute, 'last_diag', {})
...
def compute(tracks, fps, sens="보통"):
    TH = {"엄격": dict(cut=45, sig=3, opp=1),
          "보통": dict(cut=30, sig=2, opp=2),
          "민감": dict(cut=20, sig=1, opp=3)}[sens]
    ids = list(tracks)
    out = []
    low = []          # 기준 미달이지만 계산은 끝난 조합 (참고용으로 보여준다)
    diag = {"n_people": len(ids), "n_pairs": 0, "pairs": []}
    for i in range(len(ids)):
        for j in range(len(ids)):
            ...
            low_evidence = (opportunities < 2)

            # ── 동행 감점 ──
            side = 0; switches = 0; prev = None
            for k in range(1, len(com)):
                lv = (lseq[k][0]-lseq[k-1][0], lseq[k][1]-lseq[k-1][1])
                if math.hypot(*lv) < stop_thr: continue
                ux, uy = lv[0]/math.hypot(*lv), lv[1]/math.hypot(*lv)
                ...
                cur_side = "F" if along > 0 else "L"
                if prev and cur_side != prev: switches += 1
                prev = cur_side
            ded = 0
            ...
            # 반응 근거가 전혀 없으면 상한을 건다. 거리·방향·경로만으로는
            # '같은 길을 가는 사람'과 미행을 구별할 수 없기 때문이다.
            if reactions == 0:
                fin = min(fin, 29)
            ...
            for a in ls[:3]:
                ...
                elif gaps and min(gaps) <= 0.5:
                    tl.append((_ts(a, fps),
                               f"선행자 정지 → 거의 동시({min(gaps):.1f}초) 후행자 정지", "동행 신호"))
            for a in lt[:3]:
            ...
            cur = _curve_real(com, lseq, fseq, fps, scale, stop_thr, max(fin, 5))

            e_ = ev(ids[i], ids[j], fin, susp, ded, rows, tl, cur)
            e_["low_evidence"] = low_evidence
            e_["opportunities"] = opportunities
            e_["reactions"] = reactions

            # ── 게이트: 기준 통과 여부 판정 ──
            #  걸러지더라도 계산 결과는 버리지 않고 '참고' 목록에 담는다.
            why = None
            signals = sum([rr >= .5, dur >= 3, head >= .70,
                           path >= .40, stop_late >= 1, turn_late >= 1])
            if opportunities >= TH['opp'] and reactions == 0:
                why = "반응 없음"
            elif opportunities >= 4 and reactions / opportunities < 0.35:
                why = "반응률 낮음"
            elif signals < TH['sig']:
                why = f"신호 부족 ({signals}개)"
            elif fin < TH['cut']:
                why = f"점수 미달 ({fin}점)" if reactions else "반응 근거 없음"

            if why:
                e_["why"] = why
                diag["pairs"].append({"a": ids[i], "b": ids[j], "rr": rr, "dur": dur,
                                      "opp": opportunities, "react": reactions, "why": why})
                low.append(e_)
            else:
                out.append(e_)
    ...
    res = sorted(best.values(), key=lambda e: -e["score"])
    lowb = {}
    for e in low:
        k = tuple(sorted([e["leader"], e["follower"]]))
        if k not in lowb or e["score"] > lowb[k]["score"]: lowb[k] = e
    diag["low"] = sorted(lowb.values(), key=lambda e: -e["score"])
    compute.last_diag = diag
    return res
...
def report(rec):
    ...
    evs = rec["events"]
    if not evs:
        evs = (rec.get("diag") or {}).get("low", [])
        if evs:
            L += ["[참고] 기준을 넘긴 구간은 없습니다.",
                  "      아래는 계산이 완료된 참고 구간입니다.", ""]
    for e in evs:
        lv,_,_,_=level(e["score"])
        L+=[f"[{lv}] id{e['leader']} <- id{e['follower']}  ...  {e['score']}점",
            f"  의심 +{e['suspicion']} / 동행 감점 -{e['deduction']}"]
        if e.get("why"): L.append(f"  기준 미달 사유 : {e['why']}")
        L.append(f"  관찰 기회 {e.get('opportunities','-')}회 / 반응 {e.get('reactions','-')}회")
        for n,m,s in e["rows"]:
...
def page_analyze():
    ...
    if up is not None:
        ...
        if go:
            ...
            if DEMO_MODE:
                events,video,conv,dg=demo(),vp,True,{}; bar.progress(1.0,"완료")
            else:
                events,video,conv,dg=analyze_video(vp,bar,st.session_state.get("sens","보통"))
            bar.empty()
            rec={"id":str(uuid.uuid4())[:8],"name":up.name,"video":video,"events":events,
                 "at":datetime.now().strftime("%m/%d %H:%M"),"converted":conv,"diag":dg}
    ...
    events = act["events"]
    diag = act.get("diag", {}) or {}
    lows = diag.get("low", [])

    # 기준을 넘긴 구간이 없으면, 계산은 끝난 '참고 구간'을 대신 보여준다.
    fallback = False
    if not events and lows:
        events = lows
        fallback = True

    if not events:
        st.markdown(
            f"<div class='zk-card' style='border-left:5px solid {MINT};padding:20px 22px'>"
            f"<div style='font-size:16px;font-weight:700;color:{MINT_DEEP}'>"
            f"분석할 인물 관계가 없습니다</div>"
            f"<div style='font-size:13px;color:{INK_SOFT};margin-top:8px;line-height:1.7'>"
            f"영상에서 사람 {diag.get('n_people','-')}명을 추적했습니다. "
            f"두 사람이 2초 이상 함께 등장한 구간이 없어 관계를 계산하지 못했습니다."
    ...
    why=" · ".join(hits[:3]) if hits else "뚜렷한 신호 없음"
    if fallback:
        title = f"기준 미달 · id{top['leader']} 뒤에 id{top['follower']}가 있었으나 미행 근거는 약함"
        sub = f"제외 사유 {top.get('why','기준 미달')} · 참고용으로 계산 결과를 표시합니다"
    else:
        title = f"{lv} · id{top['leader']} 뒤를 id{top['follower']}가 따라간 정황"
        sub = f"주요 근거 {why}"
    st.markdown(
        ...
        f"<div style='flex:1'>"
        f"<div style='font-size:14.5px;font-weight:700;color:{deep}'>{title}</div>"
        f"<div style='font-size:12.5px;color:{INK_SOFT};margin-top:3px'>{sub}</div></div>"
    ...
    if fallback:
        st.markdown(
            f"<div style='background:{MINT_PALE};border:1px solid {MINT};border-radius:11px;"
            f"padding:11px 15px;margin:-10px 0 18px;font-size:12.5px;color:{MINT_DEEP};"
            f"line-height:1.6'>"
            f"<b>확인이 필요한 구간은 감지되지 않았습니다.</b> "
            f"아래는 기준에 미치지 못했지만 계산이 완료된 참고 구간입니다. "
            f"사이드바 민감도를 높이면 정식 결과로 표시됩니다.</div>",
    ...
    with R:
        lab("참고 구간" if fallback else "확인 필요 구간")
...
st.sidebar.markdown("<div style='height:1px;background:#33473F;margin:16px 0'></div>",
                    unsafe_allow_html=True)
st.sidebar.markdown(
    f"<div style='font-family:JetBrains Mono,monospace;font-size:10px;letter-spacing:1.2px;"
    f"color:{LEMON};margin-bottom:6px'>SENSITIVITY</div>", unsafe_allow_html=True)
SENS = st.sidebar.select_slider("민감도", options=["엄격", "보통", "민감"],
                                value="보통", label_visibility="collapsed")
st.sidebar.markdown(
    f"<div style='font-size:10.5px;color:#8FA69C;line-height:1.6;margin-top:4px'>"
    f"결과가 없으면 민감도를 올려 보세요.<br>민감할수록 오탐이 늘어납니다.</div>",
    unsafe_allow_html=True)
